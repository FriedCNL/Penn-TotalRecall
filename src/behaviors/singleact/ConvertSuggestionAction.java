...
/**
 * Converts a suggestion into an annotation; also deletes that annotation from the suggestion list
 * 
 * @author Yuvi Masory
 */
public class ConvertSuggestionAction extends IdentifiedSingleAction {

	private int rowIndex;
	private Suggestion suggToConvert;

	/**
	 * Creates an <code>Action</code> that will convert the suggestion matching the provided index argument.
	 * 
	 * @param rowIndex
	 * @param suggToConvert
	 */
	public ConvertSuggestionAction(int rowIndex) {
		this.rowIndex = rowIndex;
		this.suggToConvert = SuggestionDisplay.getSuggestionsInOrder()[rowIndex];
		this.putValue(Action.NAME, "Add Suggestion to Annotations");
	}
	...
	private static boolean deleteSuggestion(Suggestion suggToDelete, int suggRowIndex, File oFile){
		...
		if(success) {
			SuggestionDisplay.removeSuggestion(suggRowIndex);
	...
	public static File getSuggestionOutputFile() {
		String curFileName = CurAudio.getCurrentAudioFileAbsolutePath();
		File oFile = new File(OSPath.basename(curFileName) + "." + Constants.temporarySuggestionFileExtension);
		return oFile;
	}

	public static File getAnnotationOutputFile() {
		String curFileName = CurAudio.getCurrentAudioFileAbsolutePath();
	...
	public void actionPerformed(ActionEvent e) {
		...
		System.out.println("Convert action called on suggestion: " + suggToConvert.getText());
		int x  = 0;
		/*if(x == 0){
			return;
		}*/
		...
		//retrieve time associated with suggestion
		double time = suggToConvert.getTime();
		
		//retrieve text associated with annotation, possibly the intrusion string
		String text = suggToConvert.getText(); 
		...
		File oFile = getAnnotationOutputFile();
		File suggOFile = getSuggestionOutputFile();
		...
		if(oFile.exists()) {
			...
			try {
				...
				//delete the corresponding suggestion, since it is now an "official" annotation
				boolean deleteres = deleteSuggestion(suggToConvert, rowIndex, suggOFile);
			} 
			catch (IOException e1) {
				e1.printStackTrace();
	...
	public static void writeSpans() {
		if(UpdatingAction.getStamps().size() > 0) {
			...
			for(ArrayList<Long> span: spans) {
				...
				try {
					AnnotationFileParser.addField(getAnnotationOutputFile(), obfuscate(toWrite));
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
