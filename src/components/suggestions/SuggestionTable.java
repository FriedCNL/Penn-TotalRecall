...
/**
 * <code>JTable</code> that stores the Suggestions of the open audio file.
 * 
 * @author Yuvi Masory
 */
public class SuggestionTable extends JTable implements FocusListener {
	
	private static SuggestionTable instance;
	
	private static SuggestionTableModel model;
	
	private SuggestionTableCellRenderer render;

	private SuggestionTable() {
		model = new SuggestionTableModel();
		render = new SuggestionTableCellRenderer();
		...
		addMouseListener(new SuggestionTableMouseAdapter(this));
	...
	@Override
	public SuggestionTableModel getModel() {
		return model;
	}
	...
	@Override
	public SuggestionTableCellRenderer getCellRenderer(int row, int col) {
		return render;
	}
	
	@Override
	public SuggestionTableCellRenderer getDefaultRenderer(Class<?> columnClass) {
		return render;
	}
	...
	protected static SuggestionTable getInstance() {
		if(instance == null) {
			instance = new SuggestionTable();
		}
		return instance;
	}
	
	public static SuggestionTable getFocusTraversalReference() {
		return getInstance();
	}

	public static Suggestion popSelectedSuggestion() {
		int[] rows = instance.getSelectedRows();
		if(rows.length == 1) {
			return model.getSuggestionAt(rows[0]);
		}
		else {
			return null;
		}
